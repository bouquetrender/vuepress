...
function insertionSort(arr) {
  for (let i = 1; i < arr.length; ++i) {
    ...
    let j = i;
    while (j > 0) {
      if (new Date(arr[j - 1].date) < new Date(temp.date)) {
        arr[j] = arr[j - 1];
      } else {
        break;
      }
...
function renderRss(options) {
  const sortedRssItems = options.rssItems
  insertionSort(sortedRssItems)
  return `
    <rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
      <channel>
        <title>${options.siteConfig.title}</title>
        <link>https://bouquetrender.github.io</link>
...
function renderRssItem(rssItem) {
  return `
    <item>
      ...
      <link>https://bouquetrender.github.io${rssItem.path}</link>
